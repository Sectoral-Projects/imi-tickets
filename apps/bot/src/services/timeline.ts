...
export interface ListTimelineInput {
	threadId: number;
	cursor?: number;
	/** Number of complete groups to return (not raw row count). */
	limit?: number;
}
...
export interface ListTimelineWindowInput {
	...
	/** Groups per directional page / side of seed. */
	limit?: number;
	beforeLimit?: number;
	afterLimit?: number;
}
...
export abstract class TimelineService {
	static listTimeline(input: ListTimelineInput, db: DbClient = container.sqlite): ListTimelineResult {
		const groupLimit = input.limit ?? TIMELINE_GROUP_LIMIT;
		const cursor = input.cursor ?? 0;
		const fetchLimit = timelineGroupFetchRowLimit(groupLimit);
		...
		const page = takeTimelineGroups(rows, groupLimit, 'newest-first');
		...
		return { items: this.rowsToItems(input.threadId, page, db), nextCursor };
	}
	...
	static listTimelineWindow(input: ListTimelineWindowInput, db: DbClient = container.sqlite): ListTimelineWindowResult {
		...
		if (input.direction && cursor) {
			return input.direction === 'older'
				? this.listOlderWindowPage(input.threadId, cursor, groupLimit, db)
				: this.listNewerWindowPage(input.threadId, cursor, groupLimit, db);
		}
		...
		const beforeLimit = input.beforeLimit ?? TIMELINE_WINDOW_GROUP_LIMIT;
		const afterLimit = input.afterLimit ?? TIMELINE_WINDOW_GROUP_LIMIT;
		const targetCursor = toTimelineCursorParts(target);
		const olderFetch = timelineGroupFetchRowLimit(beforeLimit);
		const newerFetch = timelineGroupFetchRowLimit(afterLimit) + 1;
		...
		const completedOlderRows = takeTimelineGroups(olderRows, beforeLimit, 'newest-first');
		const completedNewerRows = takeTimelineGroups(newerRows, afterLimit + 1, 'oldest-first');
		const hasOlder = olderRows.length > completedOlderRows.length;
		const hasNewer = newerRows.length > completedNewerRows.length;
		const page = [...completedOlderRows.reverse(), ...completedNewerRows];
	...
	private static listOlderWindowPage(
		...
		const rows = db.all<TimelineRow>(sql`
			...
			LIMIT ${fetchLimit}
		`);
		const completedPage = takeTimelineGroups(rows, groupLimit, 'newest-first');
	...
	private static listNewerWindowPage(
		...
		const rows = db.all<TimelineRow>(sql`
			...
			LIMIT ${fetchLimit}
		`);
		const page = takeTimelineGroups(rows, groupLimit, 'oldest-first');
		const hasMore = rows.length > page.length;
...
function takeTimelineGroups(
	...
	return takeCompleteGroups(
		rows,
		groupLimit,
		(currentEdge: TimelineRow, candidate: TimelineRow) => {
		...
		(row: TimelineRow) => row.kind === 'audit',
		MAX_TIMELINE_ROWS_PER_PAGE
	);
}
